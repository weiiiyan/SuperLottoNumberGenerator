...
所有布局尺寸常量定义在 [mainwindow.h](src/app/mainwindow.h) 的 `public` 区：`WIDTH_TIER_*`、`LABEL_*`。彩票格式常量（`GROUP_COUNT` 等）为 `LottoPresenter::*` 的别名，展示器再别名领域层 `LottoTicket::*`（组级规则唯一来源是 `LottoGroup`），视图不直接依赖领域常量。
